...
## Overall Flow
`src/atom_agent/workspace.py` manages file system paths, resource allocation, and project structure rules for the agent. It enforces a "Workspace Contract" that ensures the agent operates within defined boundaries and uses consistent file paths.

The flow is:
1.  **Contract Definition**: The `Workspace` class defines schemas for directory structures, naming conventions, and file permissions using Pydantic.
2.  **Path Resolution**: The `get_path` method resolves complex path templates (e.g., `{step_id}/checkpoints/ckpt_{attempt_id}.json`) into concrete strings.
3.  **Staging Helpers**: `get_staging_paths` provides a standardized dictionary of paths needed for executing a single step attempt (impl.py, test.py, artifacts, logs).
...
## Edge Cases
-   **Missing Path Key**: `get_path` raises a `KeyError` if a requested template key is not defined in the contract.
...
-   **Fields**:
    ...
    -   `paths`: A dictionary of path templates (e.g., `attempt_impl`: `steps/{step_id}/attempts/{attempt_id}/impl.py`).
    -   `write_rules`, `read_rules`: Permissions.
    -   `naming`, `step_structure`, `defaults`: Configuration.

### `get_path(key: str, **kwargs) -> str`
...
### `get_staging_paths(step_id: str, attempt_id: str) -> Dict[str, str]`
...
### `from_dict(cls, data: Dict[str, Any]) -> Workspace`
